...
def show_image():#создаем функцию, которая будет выводить изображения
    ...
    if image_url:#если ссылка не пустая
        try:#обрабатываем исключения
            ...
            #получаем номер картинки-notebook.index до конца-end прибавляем единицу,
            # чтобы номер картинки начинался не  с нуля, а с единицы
            lb = ttk.Label(tab, image=img)#создаем новую метку для нового окна(tab).
            # параметр-image равен тому где лежит наша картинка-img
            lb.pack(padx=10, pady=10)#добавляем отступы-padx=10 и pady=10
...
notebook = ttk.Notebook(top_level_window)#создаем виджет-notebook в окне-top_level_window
notebook.pack(expand=True, fill='both', padx=10, pady=10)#expand=True-чтобы заполнить все окно
